...
function extractBedsFromText(text: string): string[] {
  ...
  if (!beds.length && /病区|全部患者|所有患者|全病区/.test(raw)) {
    return [];
  }
  return beds;
}
...
const mockTemplates: DocumentTemplate[] = [
  ...
  },
  {
    ...
    trigger_keywords: ["病危护理记录", "病重护理记录"],
    template_text:
      "【病重（病危）患者护理记录】\n患者姓名：{{patient_name}}\n床号：{{bed_no}}\n主要诊断：{{diagnoses}}\n生命体征：{{observation_summary}}\n重点风险：{{risk_tags}}\n护理措施与效果：{{spoken_text}}\n下一班观察要点：{{pending_tasks}}",
    created_by: "system",
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  ...
  },
  {
    ...
    trigger_keywords: ["血糖测量记录", "血糖单", "POCT"],
    template_text:
      "【血糖测量记录单（POCT）】\n患者姓名：{{patient_name}}\n床号：{{bed_no}}\n测量时间：{{current_time}}\n测量项目：{{extra_measurement_item}}\n血糖值：{{glucose_value}}\n补充：{{spoken_text}}",
    created_by: "system",
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    ...
    trigger_keywords: ["交接班报告", "交班报告"],
    template_text:
      "【护理日夜交接班报告】\n班次：{{shift_type}}\n患者姓名：{{patient_name}}\n床号：{{bed_no}}\n诊断：{{diagnoses}}\n本班主要变化：{{observation_summary}}\n本班护理措施：{{spoken_text}}\n下一班观察重点：{{pending_tasks}}",
    created_by: "system",
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
...
export const mockApi = {
  ...
  async getBedContext(bedNo: string, _departmentId?: string) {
    await sleep(250);
    const bed = mockBeds.find((item) => item.bed_no === bedNo);
    if (!bed?.current_patient_id) {
      throw new Error("bed_context_not_found");
    }
    return mockContexts[bed.current_patient_id];
  },
    ...
    return {
      single_models: [
        ...
        },
        {
          ...
          name: "中医问诊（本地）",
          provider: "local",
          description: "补充中医辨证线索和中医特色护理观察点",
        },
        {
          id: "bailian_main",
          name: "阿里百炼主模型",
      ...
      cluster_profiles: [
        ...
        },
        {
          ...
          name: "中医特色协同",
          main_model: "中医问诊（本地）",
          description: "在护理主流程上叠加中医辨证辅助，适合做中医特色护理提示。",
          tasks: [
            {
              model_id: "care-planner",
              model_name: "处理顺序整理",
  ...
  async previewPatientScope(payload: {
    ...
    await sleep(120);
    const extracted = extractBedsFromText(payload.userInput);
    if (payload.bedNo && !extracted.includes(payload.bedNo)) {
      extracted.unshift(payload.bedNo);
    }

    const wardScope = /病区|全部患者|所有患者|全病区/.test(payload.userInput || "") || extracted.length > 1;
    const matchedPatients = wardScope
      ? Object.values(mockContexts).slice(0, 6).map((ctx) => ({
          patient_id: ctx.patient_id,
          patient_name: mockPatients[ctx.patient_id]?.full_name,
          bed_no: ctx.bed_no,
          diagnoses: ctx.diagnoses || [],
          risk_tags: ctx.risk_tags || [],
          pending_tasks: ctx.pending_tasks || [],
          requested_bed_no: ctx.bed_no,
          resolved_bed_no: ctx.bed_no,
          bed_no_corrected: false,
        }))
      : extracted
          .map((bedNo) => {
            const context = Object.values(mockContexts).find((ctx) => String(ctx.bed_no) === String(bedNo));
            if (!context) {
              return null;
            }
            return {
              patient_id: context.patient_id,
              patient_name: mockPatients[context.patient_id]?.full_name,
              bed_no: context.bed_no,
              diagnoses: context.diagnoses || [],
              risk_tags: context.risk_tags || [],
              pending_tasks: context.pending_tasks || [],
              requested_bed_no: bedNo,
              resolved_bed_no: context.bed_no,
              bed_no_corrected: false,
            };
          })
          .filter(Boolean);

    const resolvedRequestedBeds = new Set(
      matchedPatients.flatMap((item: any) => [item.requested_bed_no, item.resolved_bed_no, item.bed_no].filter(Boolean))
    );

    return {
      question: payload.userInput,
      department_id: payload.departmentId || "dep-card-01",
      ward_scope: wardScope,
      global_scope: /全院|全库|数据库/.test(payload.userInput || ""),
      extracted_beds: extracted,
      unresolved_beds: extracted.filter((bed) => !resolvedRequestedBeds.has(bed)),
      matched_patients: matchedPatients as PatientScopePreview["matched_patients"],
    };
  },
  ...
  async runAiChat(payload: {
    ...
    await sleep(620);
    const extractedBeds = extractBedsFromText(payload.userInput);
    const wardScope = /病区|全部患者|所有患者|全病区/.test(payload.userInput || "") || extractedBeds.length > 1;
    const hasExplicitScope = Boolean(payload.patientId || payload.bedNo || extractedBeds.length || wardScope);
    const resolvedBedNo = hasExplicitScope ? payload.bedNo || extractedBeds[0] || undefined : undefined;
    const resolvedContext = resolvedBedNo
      ? Object.values(mockContexts).find((ctx) => String(ctx.bed_no) === String(resolvedBedNo))
      : undefined;
    const patientId = hasExplicitScope ? payload.patientId || resolvedContext?.patient_id || "pat-001" : undefined;
    const patientName = patientId ? mockPatients[patientId]?.full_name : undefined;
    const profile = normalizeExecutionProfile(payload.executionProfile);
    ...
    const dynamic =
      payload.selectedModel === "tcm_consult_local"
        ? {
            summary: hasExplicitScope
              ? `${resolvedBedNo || "-"}床${patientName ? `（${patientName}）` : ""}已进入中医辅助问诊模式，建议围绕寒热虚实、舌苔脉象和睡眠食纳补充问询，并结合西医生命体征人工复核。`
              : "已进入通用中医护理问答模式，可直接询问中医辨证思路、中医护理观察点或中西医结合照护建议；只有结合具体患者时才需要补充床号。",
            findings: hasExplicitScope
              ? ["已切换中医问诊模型", "建议补充寒热、口渴、纳差、二便、睡眠等中医问诊要点"]
              : ["已切换中医问诊模型", "当前未绑定具体病例，本次按通用中医护理问答处理"],
            recommendations: [
              { title: "补充中医辨证线索后再形成护理观察重点", priority: 1 },
              { title: "涉及方药与处方必须转中医师确认", priority: 1 },
            ],
          }
        : dynamicSummary(plannerText || payload.userInput, profile);
    const workflowType =
    ...
    if (!hasExplicitScope) {
      const summary = withMissionSummary(dynamic.summary, payload.missionTitle);
      const genericPlan =
        payload.mode === "single_model"
          ? [
              {
          ...
          : [
              ...
              },
              {
                ...
                reason: "当前问题无需绑定病例，直接按通用护理助手模式回答。",
                status: "done",
                auto_runnable: true,
              },
            ];

      const genericArtifacts: AIChatResponse["artifacts"] = /文书|模板|导入|护理记录|记录单|草稿|document|template/i.test(
        plannerText || payload.userInput
      )
        ? [
            {
              ...
              title: "文书模板建议",
              status: "created",
              reference_id: `template-${Date.now()}`,
              summary: "当前问题更适合走文书模板导入或模板编辑，而不是绑定具体病例生成草稿。",
      ...
      const genericMemory = {
        conversation_summary: "当前会话未绑定具体病例，按通用护理助手模式保留上下文。",
        patient_facts: [],
        unresolved_tasks: [],
        last_actions: ["已切换到通用问答"],
        user_preferences: [
          "倾向于先直接回答，不强制先选病例",
          /中医|辨证/.test(plannerText || payload.userInput) ? "中医问题优先补充中医护理视角" : "需要高效率简洁说明",
        ],
      };
      ...
      const nextActions = [
        "继续直接提问即可",
        "涉及具体患者时再补充床号或病区",
        ...dynamic.recommendations.map((item) => item.title),
      ].slice(0, 4);
      ...
      const response: AIChatResponse = {
        ...
        run_id: `run-${Date.now()}`,
        runtime_engine: buildRuntimeStatus().active_engine,
        workflow_type: payload.mode === "single_model" ? "single_model_chat" : "voice_inquiry",
        summary,
        findings: ["当前未绑定具体病例，本次按通用问答处理。", ...dynamic.findings],
        recommendations: [
          { title: "继续直接提问，不需要先选病例。", priority: 1 },
          { title: "如果要结合某位患者，再补充床号或病区。", priority: 2 },
          ...dynamic.recommendations,
        ].slice(0, 4),
        confidence: payload.mode === "single_model" ? 0.75 : 0.82,
        review_required: true,
        steps:
          payload.mode === "single_model"
            ? [{ agent: "Single Model Runner", status: "done", input: { selected_model: payload.selectedModel || "minicpm3_4b_local" }, output: {} }]
            : [
                { agent: "Intent Router Agent", status: "done", input: {}, output: { workflow_type: "voice_inquiry" } },
                { agent: "General Assistant Agent", status: "done", input: {}, output: { scoped: false } },
              ],
        ...
        agent_goal:
          payload.mode === "single_model"
            ...
            ? `围绕任务「${payload.missionTitle}」推进通用护理问答`
            : "按 AI Agent 模式处理通用护理、系统和中医相关问题",
        agent_mode:
          payload.mode === "single_model"
            ? "direct_answer"
        ...
        specialist_profiles: [
          {
            ...
            title: "通用护理助手",
            role: "通用问答",
            focus: "回答护理流程、系统能力、文书逻辑和基础中医护理问题。",
            status: "active",
            reason: "当前问题不需要绑定具体病例。",
        ...
        hybrid_care_path: [
          {
            ...
            status: "done",
            owner: "通用护理助手",
            summary: "已识别为无需绑定病例的通用问题。",
          },
          {
            id: "general_response",
            title: "直接回答",
            status: "done",
            owner: payload.mode === "single_model" ? "单模型" : "AI Agent",
            summary: "已按通用问答模式返回结果。",
          },
        ],
        data_capsule: {
          patient_id: undefined,
          version: "general-session",
          event_summary: ["当前未绑定具体患者", "本次按通用护理问答处理"],
          time_axis: [],
          data_layers: ["general_knowledge", "workflow_memory"],
          risk_factors: [],
        },
        health_graph: {
          nodes: ["通用护理问答", "文书中心", "中医护理", "AI Agent"],
          edges: ["通用问题 -> 直接回答", "提到床号 -> 自动挂病例", "文书模板 -> 收件箱处理"],
          dynamic_updates: ["当前无需病例上下文"],
        },
        reasoning_cards: [
          {
            mode: "general",
            title: "通用问答路径",
            summary: "当前问题没有床号或病区范围，因此直接走通用护理助手回答。",
            confidence: 0.84,
          },
        ],
        pending_approvals: [],
    ...
    const scopedPatientId = patientId || "pat-001";
    const memory = buildAgentMemory(scopedPatientId, plannerText || payload.userInput, profile);
    const artifacts = buildAgentArtifacts(scopedPatientId, plannerText || payload.userInput, plan);
    const steps = buildAgentSteps({
      mode: payload.mode,
    ...
    const response: AIChatResponse = {
      ...
      agent_goal:
        payload.missionTitle
          ? `围绕${mockContexts[scopedPatientId]?.bed_no || payload.bedNo || "-"}床推进任务「${payload.missionTitle}」`
          : payload.mode === "single_model"
          ? `用单模型直接回答${mockContexts[scopedPatientId]?.bed_no || payload.bedNo || "-"}床问题`
          : `围绕${mockContexts[scopedPatientId]?.bed_no || payload.bedNo || "-"}床执行${EXECUTION_PROFILE_LABELS[profile]}任务`,
      agent_mode:
        payload.mode === "single_model"
          ? "direct_answer"
          : payload.agentMode || profile === "full_loop" || /自动|闭环|自治/.test(payload.userInput)
  ...
  async createDocumentDraft(
    patientId: string,
    text: string,
    options?: { documentType?: string; templateId?: string; templateText?: string; templateName?: string }
    ...
    const patient = mockPatients[patientId];
    const context = mockContexts[patientId];
    const observationSummary = (context?.latest_observations || [])
    ...
    const rendered = templateText
      ? templateText
          ...
          .replace("{{bed_no}}", context?.bed_no || "-")
          .replace("{{mrn}}", patient?.mrn || "-")
          .replace("{{inpatient_no}}", patient?.inpatient_no || "-")
          .replace("{{gender}}", patient?.gender || "待补充")
          .replace("{{age}}", patient?.age ? String(patient.age) : "待补充")
          .replace("{{blood_type}}", patient?.blood_type || "待补充")
          .replace("{{chart_date}}", new Date().toISOString().slice(0, 10))
          .replace("{{current_time}}", new Date().toISOString().slice(0, 16).replace("T", " "))
          .replace("{{diagnoses}}", (context?.diagnoses || []).join("、"))
          .replace("{{risk_tags}}", (context?.risk_tags || []).join("、"))
          .replace("{{pending_tasks}}", (context?.pending_tasks || []).join("、"))
          .replace("{{observation_summary}}", observationSummary || "待补充")
          .replace("{{spoken_text}}", spoken)
          .replace("{{shift_type}}", "day")
          .replace("{{extra_measurement_item}}", "随机血糖")
          .replace("{{glucose_value}}", "待补充")
          .replace("{{transfusion_reaction}}", "待补充")
          .replace("{{transfusion_start_time}}", "待补充")
          .replace("{{transfusion_end_time}}", "待补充")
          .replace("{{operation_name}}", "待补充")
          .replace("{{instrument_count}}", "待补充")
          .replace("{{dressing_count}}", "待补充")
          .replace("{{special_item_count}}", "待补充")
      : `[护理记录]\n患者ID: ${patientId}\n内容: ${spoken}`;

    const item: DocumentDraft = {
      id: `draft_${Date.now()}`,
      patient_id: patientId,
      document_type: chosenTemplate?.document_type || options?.documentType || "nursing_note",
      draft_text: `${rendered}\nAI提示: 需人工复核。`,
      status: "draft",
      structured_fields: {
  ...
  async getDocumentInbox(_requestedBy: string, options?: { patientId?: string; limit?: number }) {
    await sleep(220);
    const rows = options?.patientId ? mockDrafts.filter((item) => item.patient_id === options.patientId) : [...mockDrafts];
  ...
  async listWorkflowHistory(options?: { conversationId?: string; limit?: number }) {
    await sleep(180);
    let rows = [...mockConversationHistory];
    if (options?.conversationId) {
      rows = rows.filter((item) => item.conversation_id === options.conversationId);
    }
