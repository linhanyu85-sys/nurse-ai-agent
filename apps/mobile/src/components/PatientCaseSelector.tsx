...
export function PatientCaseSelector({ departmentId, selectedPatient, onSelectPatient, onCasesUpdated }: Props) {
  ...
  const filteredCases = useMemo(() => {
    ...
    return cases.filter((item) => {
      const haystack = [
        formatBedLabel(item.bed_no, ""),
        normalizePersonName(item.patient_name),
  ...
  return (
    <SurfaceCard>
      <View style={styles.header}>
        <Text style={styles.title}>病例列表（可直接切换当前病例或进入患者档案）</Text>
      ...
      {selectedPatient ? (
        <View style={styles.selectedWrap}>
          ...
          <Text style={styles.selectedText}>
            {normalizePersonName(selectedPatient.full_name, selectedPatient.id)}（ID: {selectedPatient.id}）
          </Text>
        </View>
      ) : (
          ...
          >
            <View style={styles.caseTop}>
              <Text style={styles.caseName}>
                {formatBedLabel(item.bed_no)} · {normalizePersonName(item.patient_name, patientId)}
              </Text>
